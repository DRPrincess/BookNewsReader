...
public class HttpKLogingInterceptor implements Interceptor {
    ...
    private volatile HttpKLogingInterceptor.Level level = HttpKLogingInterceptor.Level.NONE;

    /** Change the level at which this interceptor logs. */
    public HttpKLogingInterceptor setLevel(HttpKLogingInterceptor.Level level) {
        if (level == null) throw new NullPointerException("level == null. Use Level.NONE instead.");
        this.level = level;
        return this;
    }

    public HttpKLogingInterceptor.Level getLevel() {
        return level;
    }

    @Override public Response intercept(Chain chain) throws IOException {
        HttpKLogingInterceptor.Level level = this.level;

        Request request = chain.request();
        if (level == HttpKLogingInterceptor.Level.NONE) {
            return chain.proceed(request);
        }

        boolean logBody = level == HttpKLogingInterceptor.Level.BODY;
        boolean logHeaders = logBody || level == HttpKLogingInterceptor.Level.HEADERS;
